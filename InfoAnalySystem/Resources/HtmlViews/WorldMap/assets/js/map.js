...
function addEvent(id,relationId){
	...
	if(map_config[id]['enable'] == true){
		...
		_Textobj.click(function(){
			if(!$('#'+id).attr('countrySelected')){
				$('#'+id).css({'fill':map_config[id]['downColor']});
				$('path[countrySelected]').css({'fill':map_config[id]['upColor']});
				$('path[countrySelected]').attr('countrySelected',null);
				$('#'+id).attr('countrySelected',true);
			}
		})
